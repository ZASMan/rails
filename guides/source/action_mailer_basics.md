...
In the case that you have a Rails application with an unorthodox file organization and wish to render a template located outside of the default `app/views/mailer_name/` directory, you can apply the `prepend_view_path`, like so:
...
class UserMailer < ApplicationMailer
  ...
  def welcome_email
    @user = params[:user]
    full_template_path = Rails.root.to_s + "/custom_folder/user_mailer/views/"
    prepend_view_path(full_template_path)
    mail(
      to: @user.email,
      subject: subject,
